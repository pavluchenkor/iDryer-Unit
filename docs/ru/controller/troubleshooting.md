# Проблемы со связью iDryer Unit и их решение

При использовании **iDryer Unit** в некоторых случаях могут возникать проблемы со стабильностью соединения (обрывы, «потеря» MCU, нестабильная работа).  
...
!!! info "Причина"
    Вибрации от принтера или самой сушилки могут вызывать микроподвижки USB-разъёма, что приводит к кратковременной потере контакта.
...
!!! warning "Симптомы"
    ...
    - Сбои, совпадающие с работой вентилятора сушилки  
    - Нестабильность при ШИМ-управлении  

!!! info "Причина"
    Вентилятор 110–230 В оснащён импульсным блоком питания и может создавать помехи, влияющие на сигнальные линии.

!!! success "Решение"
    Рекомендуется установить **RC-снаббер (snubber)** параллельно вентилятору. Или использовать ферритовый фильтр на USB-кабель.
...
!!! success "Решение"
    - Подключайте iDryer Unit **только в порты USB 2.0** (обычно чёрные разъёмы)  
...
!!! warning "Симптомы"
    - Контроллер не определяется в режиме bootloader (BOOTSEL)  
    - Прошивка завершается ошибкой или зависает  
    - Устройство определяется, но запись образа не проходит  
